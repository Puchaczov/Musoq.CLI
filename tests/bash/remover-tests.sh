...
mkdir -p "$test_root/bin" "$INSTALL_DIR"
printf 'binary' > "$MUSOQ_EXE"
ln -s "$MUSOQ_EXE" "$test_root/bin/Musoq"
printf 'external' > "$test_root/bin/musoq"
...
assert_succeeds "regular file is left alone" remove_managed_symlink "$test_root/bin/musoq"
assert_equal external "$(<"$test_root/bin/musoq")" "non-symlink collision is preserved"
